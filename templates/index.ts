...
const requestContext = {
  kangaroo: '/kangaroo-library-center',
};
export default class MyGenerator extends CodeGenerator {
  ...
  getInterfaceContent(inter: Interface) {
    ...
    const requestParams = requestArgs.join(', ');
    const basicUrl = requestContext[this.dataSource.name];
    ...
    export function request(${requestParams}) {
      ...
      options)
      return ajax('${basicUrl}${inter.path}', option);
    }
   `;
  }
...
export class FileStructures extends pont.FileStructures {
  ...
  getDataSourcesDeclarationTs() {
    const dsNames: string[] = (this as any).generators.map(ge => ge.dataSource.name);
